...
class MyEntriesFragment() : Fragment(), MyEntriesRecyclerViewAdapter.ItemClickListener {
    ...
    override fun onCreateView(
        ...
        adapter = MyEntriesRecyclerViewAdapter(this.requireContext(), viewModel.entries.value, this)
    ...
    private fun deleteEntry(position: Int) {
        CoroutineScope(IO).launch {
            ((activity as MenuActivity).application as BitfitApplication).db.fitnessDao().deleteThis(
                viewModel.entries.value?.get(position)
            )
        }
